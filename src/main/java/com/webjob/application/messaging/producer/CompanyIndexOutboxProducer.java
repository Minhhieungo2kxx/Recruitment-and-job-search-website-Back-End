...
@Slf4j
public class CompanyIndexOutboxProducer {
    ...
    public void publishEvents() {
        ...
        log.info("Starting CompanyIndex scheduled batch publication of {} outbox events", events.size());
        for (OutboxEvent event : events) {
            publish(event);
        }
        log.info("Finished CompanyIndex scheduled batch publication cycle");
    }
    ...
    private void publish(OutboxEvent event) {
        ...
        try {
            ...
            log.debug("Successfully CompanyIndex  published  outbox event - ID: {}, Exchange: {}, RoutingKey: {}",
                    event.getId(), event.getExchangeName(), event.getRoutingKey());


        } catch (Exception e) {
            log.error("Failed CompanyIndex to publish outbox event id={}", event.getId(), e);
